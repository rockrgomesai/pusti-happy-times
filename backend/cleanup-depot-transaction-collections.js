...
async function cleanupDepotTransactionCollections() {
  try {
    ...
    const hasDepotTransactionsIn = (await db.listCollections({ name: 'depot_transactions_in' }).toArray()).length > 0;
    const hasDepottransactionins = (await db.listCollections({ name: 'depottransactionins' }).toArray()).length > 0;
    
    if (hasDepottransactionins) {
      const mainCount = hasDepotTransactionsIn ? await db.collection('depot_transactions_in').countDocuments() : 0;
    ...
    const hasDepotTransactionsOut = (await db.listCollections({ name: 'depot_transactions_out' }).toArray()).length > 0;
    const hasDepottransactionouts = (await db.listCollections({ name: 'depottransactionouts' }).toArray()).length > 0;
    
    if (hasDepottransactionouts) {
      const mainCount = hasDepotTransactionsOut ? await db.collection('depot_transactions_out').countDocuments() : 0;
