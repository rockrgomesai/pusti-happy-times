...
async function searchAllCollections() {
  try {
    ...
    const requisitionCollections = collections.filter(
      (c) => c.name.toLowerCase().includes("requisition") || c.name.toLowerCase().includes("req")
    );
    ...
    for (const coll of requisitionCollections) {
      ...
      if (count > 0) {
        const sample = await db.collection(coll.name).find().limit(3).toArray();
    ...
    for (const coll of collections) {
      for (const reqNo of reqNumbers) {
        const doc = await db.collection(coll.name).findOne({
          $or: [{ requisition_no: reqNo }, { req_no: reqNo }, { number: reqNo }, { code: reqNo }],
        });
