...
async function checkDepotTransactionCollections() {
  try {
    ...
    console.log("\n🔵 IN Transactions:");
    console.log(
      `   depot_transactions_in (correct): ${hasDepotTransactionsIn ? "✅ EXISTS" : "❌ NOT FOUND"}`
    );
    console.log(
      `   depottransactionins (duplicate): ${hasDepottransactionins ? "⚠️  EXISTS" : "✅ CLEAN"}`
    );

    console.log("\n🔴 OUT Transactions:");
    console.log(
      `   depot_transactions_out (correct): ${hasDepotTransactionsOut ? "✅ EXISTS" : "❌ NOT FOUND"}`
    );
    console.log(
      `   depottransactionouts (duplicate): ${hasDepottransactionouts ? "⚠️  EXISTS" : "✅ CLEAN"}`
    );
    console.log("");
