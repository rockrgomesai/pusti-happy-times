/**
 * Check shabnamprod user details and permissions
 */
...
async function checkUser() {
  try {
    ...
    // 1. Find shabnamprod user
    const usersCollection = db.collection("users");
    const user = await usersCollection.findOne({ username: "shabnamprod" });

    if (!user) {
      console.log("❌ User shabnamprod not found");
      process.exit(1);
    }
    ...
    // 4. Check for production:send-to-store:create specifically
    const hasCreate = permissionDetails.includes("production:send-to-store:create");
    console.log(`\n${hasCreate ? "✅" : "❌"} Has production:send-to-store:create: ${hasCreate}`);
