    ...
    for (const outletId of route.outlet_ids) {
      ...
      if (outlet) {
        ...
        if (outlet.location && outlet.location.coordinates) {
          console.log(`   GeoJSON: [${outlet.location.coordinates[0]}, ${outlet.location.coordinates[1]}]`);
          console.log(`   (lng, lat): (${outlet.location.coordinates[0]}, ${outlet.location.coordinates[1]})`);
        }
        console.log();
      } else {
