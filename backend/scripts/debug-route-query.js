...
async function debugRouteQuery() {
  try {
    ...
    if (routeByRouteId) {
      console.log(
        "   SR1 ID:",
        routeByRouteId.sr_assignments?.sr_1?.sr_id,
        "Type:",
        typeof routeByRouteId.sr_assignments?.sr_1?.sr_id
      );
    ...
    const allUserRoutes = await Route.find({
      $or: [{ "sr_assignments.sr_1.sr_id": userIdObj }, { "sr_assignments.sr_2.sr_id": userIdObj }],
    });
