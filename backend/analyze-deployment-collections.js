...
async function analyzeCollectionsForDeployment() {
  try {
    ...
    for (const coll of collections) {
      ...
      if (
        name.includes("api_permission") ||
        name.includes("pg_permission") ||
        name.includes("sidebar_menu") ||
        name === "roles"
      ) {
        ...
        name.includes("order") ||
        name.includes("chalan") ||
        name.includes("invoice") ||
        name.includes("transaction") ||
        name.includes("stock") ||
        name.includes("ledger") ||
        name.includes("loadsheet") ||
        name === "loadsheets"
      ) {
    ...
    for (const category of Object.values(categories)) {
      ...
      for (const coll of category.collections) {
        ...
        console.log(
          `   ${coll.name.padEnd(40)} ${status.padEnd(20)} ${coll.sampleFields.substring(0, 30)}`
        );
      }
    }
    ...
    console.log("-".repeat(100));
    [...categories.systemData.collections, ...categories.businessMaster.collections].forEach(
      (c) => {
    ...
    console.log("-".repeat(100));
    [
      ...categories.transactional.collections,
      ...categories.userSpecific.collections,
      ...categories.testData.collections,
    ].forEach((c) => {
