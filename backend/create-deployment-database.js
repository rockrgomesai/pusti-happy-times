...
async function createDeploymentDatabase() {
  try {
    ...
    for (const collInfo of collections) {
      ...
      for (const index of indexes) {
        if (index.name !== '_id_') {
          ...
          await mongoose.connection.client.db(backupDbName).collection(collName).createIndex(keys, options);
        }
      }
    }
    ...
    const superadmin = users.find(u => u.username === 'superadmin' || u.role_id?.toString() === '68be2193ea73210503fa3329');
    ...
    console.log(`      mongorestore --host localhost --port 27017 --db=pusti_happy_times ./deployment_backup/${deploymentDbName}`);
    console.log('');
    console.log('   OR use MongoDB Atlas/Cloud:');
    console.log('      mongorestore --uri="mongodb+srv://user:pass@cluster.mongodb.net/pusti_happy_times" ./deployment_backup/${deploymentDbName}');
