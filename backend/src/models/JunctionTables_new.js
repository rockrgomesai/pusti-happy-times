...
roleSidebarMenuItemSchema.index(
  { role_id: 1, sidebar_menu_item_id: 1 },
  { unique: true }
);
...
roleApiPermissionSchema.index(
  { role_id: 1, api_permission_id: 1 },
  { unique: true }
);
 ...
 */
roleSidebarMenuItemSchema.statics.getMenuItemsForRole = async function (
  roleId
) {
  try {
 ...
 */
roleSidebarMenuItemSchema.statics.hasMenuAccess = async function (
  roleId,
  menuItemId
) {
  try {
 ...
 */
roleApiPermissionSchema.statics.getPermissionsForRole = async function (
  roleId
) {
  try {
    ...
    throw new Error(
      `Error fetching API permissions for role: ${error.message}`
    );
  }
};
 ...
 */
roleApiPermissionSchema.statics.hasApiPermission = async function (
  roleId,
  permissionName
) {
  try {
    ...
    const permission = await ApiPermission.findOne({
      api_permissions: permissionName,
    });
 ...
 */
rolePagePermissionSchema.statics.getPermissionsForRole = async function (
  roleId
) {
  try {
    ...
    throw new Error(
      `Error fetching page permissions for role: ${error.message}`
    );
  }
};
 ...
 * @returns {Boolean} True if role has permission, false otherwise
 */
rolePagePermissionSchema.statics.hasPagePermission = async function (
  roleId,
  permissionName
) {
  try {
    ...
    const access = await this.findOne({
      role_id: roleId,
      pg_permission_id: permission._id,
    });
    return !!access;
  } catch (error) {
    return false;
...
const RoleSidebarMenuItem = mongoose.model(
  "RoleSidebarMenuItem",
  roleSidebarMenuItemSchema
);
const RoleApiPermission = mongoose.model(
  "RoleApiPermission",
  roleApiPermissionSchema
);
const RolePagePermission = mongoose.model(
  "RolePagePermission",
