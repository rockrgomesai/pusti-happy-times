...
router.get('/chalans/receive-list', authenticate, requireApiPermission('distributor-chalan:read'), async (req, res) => {
  try {
    ...
    // Get total count
    const total = await models.DeliveryChalans.countDocuments(query);

    // Get paginated chalans
    const chalans = await models.DeliveryChalans.find(query)
...
router.get('/chalans/:id/receive-details', authenticate, requireApiPermission('distributor-chalan:read'), async (req, res) => {
  try {
    ...
    // Get chalan
    const chalan = await models.DeliveryChalans.findOne({
      _id: id,
      distributor_id: distributorId,
      status: 'Delivered'
    })
...
router.post('/chalans/:id/receive', authenticate, requireApiPermission('distributor-chalan:receive'), async (req, res) => {
  ...
  try {
    ...
    if (notes) {
      chalan.notes = chalan.notes ? `${chalan.notes}\n\nReceipt Notes: ${notes}` : `Receipt Notes: ${notes}`;
    }
