...
async function cleanupDuplicateCollection() {
  try {
    ...
    console.log(
      `   api_permissions (correct): ${hasApiPermissions ? "✅ EXISTS" : "❌ NOT FOUND"}`
    );
    ...
    const validDuplicateDocs = duplicateDocs.filter(
      (doc) => doc.api_permissions && doc.api_permissions.trim()
    );
    const invalidDuplicateDocs = duplicateDocs.filter(
      (doc) => !doc.api_permissions || !doc.api_permissions.trim()
    );

    const duplicatesToMigrate = validDuplicateDocs.filter(
      (doc) => !mainKeys.has(doc.api_permissions)
    );
    const duplicatesAlreadyExist = validDuplicateDocs.filter((doc) =>
      mainKeys.has(doc.api_permissions)
    );
    ...
    if (invalidDuplicateDocs.length > 0) {
      ...
      invalidDuplicateDocs.forEach((doc, i) => {
        console.log(
          `   ${i + 1}. _id: ${doc._id}, api_permissions: ${doc.api_permissions || "undefined"}`
        );
      });
      console.log("");
    }
