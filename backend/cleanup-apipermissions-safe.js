...
async function cleanupDuplicateCollection() {
  try {
    ...
    // Filter out invalid documents (undefined or missing api_permissions)
    const validDuplicateDocs = duplicateDocs.filter(doc => doc.api_permissions && doc.api_permissions.trim());
    const invalidDuplicateDocs = duplicateDocs.filter(doc => !doc.api_permissions || !doc.api_permissions.trim());
    
    const duplicatesToMigrate = validDuplicateDocs.filter(doc => !mainKeys.has(doc.api_permissions));
    const duplicatesAlreadyExist = validDuplicateDocs.filter(doc => mainKeys.has(doc.api_permissions));
    
    console.log('');
    console.log('📈 Migration Analysis:');
    console.log(`   Invalid documents (will be discarded): ${invalidDuplicateDocs.length}`);
    console.log(`   Valid documents unique to duplicate: ${duplicatesToMigrate.length}`);
    console.log(`   Valid documents already in main: ${duplicatesAlreadyExist.length}`);
    console.log('');
    
    if (invalidDuplicateDocs.length > 0) {
      console.log('🗑️  Invalid documents to be discarded:');
      invalidDuplicateDocs.forEach((doc, i) => {
        console.log(`   ${i + 1}. _id: ${doc._id}, api_permissions: ${doc.api_permissions || 'undefined'}`);
      });
      console.log('');
    }
