...
export default function TransferReceiveListPage() {
  ...
  useEffect(() => {
    loadTransfers();
  }, [page, rowsPerPage, status, startDate, endDate, fromDepotId]);
  ...
  const loadTransfers = async () => {
    try {
      ...
      if (fromDepotId) params.from_depot_id = fromDepotId;
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          View all incoming depot transfers
        </Typography>
      </Box>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell><strong>From Depot</strong></TableCell>
              ...
              {loading ? (
                ...
                transfers.map((transfer) => (
                  <TableRow key={transfer._id} hover>
                    ...
                    <TableCell>
                      {transfer.from_depot_id.name}
                      <Typography variant="caption" display="block" color="text.secondary">
                        {transfer.from_depot_id.code}
                      </Typography>
                    </TableCell>
          ...
          {loadingDetails ? (
            ...
            <>
              <Grid2 container spacing={2} sx={{ mb: 3 }}>
                ...
                {transferDetails.received_by && (
                  <>
                    <Grid2 size={{ xs: 12, sm: 6 }}>
                      <Typography variant="body2" color="text.secondary">Received By:</Typography>
                      <Typography variant="body1">{transferDetails.received_by.name}</Typography>
                    </Grid2>
                    <Grid2 size={{ xs: 12, sm: 6 }}>
                      <Typography variant="body2" color="text.secondary">Received At:</Typography>
                      <Typography variant="body1">
                        {formatDateForDisplay(transferDetails.received_at)}
                      </Typography>
                    </Grid2>
                  </>
                )}
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    {transferDetails.items.map((item: any, index: number) => {
                      const isFullyReceived = item.qty_received === item.qty_sent;
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell align="right">
                            <Chip
                              label={isFullyReceived ? "Complete" : "Partial"}
                              color={isFullyReceived ? "success" : "warning"}
                              size="small"
                            />
                          </TableCell>
                        </TableRow>
